...
class Pipeline:
    def __init__(self):
        ...
        self._raw_queue = multiprocessing.Queue(1)
        self._rgb_queue = multiprocessing.Queue(1)
    ...
    def create_node_pdaf(self):
        raw_processor=node.Node('PDAF',self._input_queue, self._result_queue)
        self._nodes.append(raw_processor)
    ...
    def initialize(self,path:"list or str", plugin_init_data_list):
        ...
        for node in self._nodes:
            ...
            node.initialize(self._input_queue.qsize(), plugin_init_data_list)
    def process(self):
        for node in self._nodes:
            node.start()
