...
import copy as cp
from . import check_rules as cr
from ..raw2rgb.mipiraw2raw import Mipiraw2Raw
from ..raw2rgb.fileoperations import test_run_time
...
class PDSplit(object):
    ...
    def pdbuffer_type_check(self):
        if(bool(1-(cr.get_bits_type(self.buffertype)))):
    ...
    def run(self): 
        ...
        #print ((PDraw_all,H_PDAll,W_PDAll))
...
def main():
    ...
    parser.add_argument('--path_raw',type=str,  default='OriginalRaw.raw' ,help='original raw source file path include PD piexl')
    ...
    #python3 PDSplit.py --XMlpath 'ov08a10_pdaf.xml' --path_raw 'OriginalRaw.raw' --rawtype 'raw' --width 3264 --height 2448 --bits 10 --buffertype LR
if __name__ == "__main__":
        main()

pdsplit_processor = None
XMlpath = None
path_raw = None
rawtype =None
width = None
...
def setParameters(args):
    ...
    global XMlpath, path_raw, rawtype, width, height, bits, buffertype, pdsplit_processor
    local_params = [XMlpath, path_raw, rawtype, width, height, bits, buffertype]
    # print(args)
    args.pop()
    i=0
    while i < len(args):
    ...
    pdsplit_processor = PDSplit(local_params[0], 
                                local_params[1],
                                local_params[2], 
                                local_params[3],
                                local_params[4], 
                                ...
                                local_params[6])
def run(buffer):
    global pdsplit_processor
    return pdsplit_processor.run()